...
interface WorkListDataProvider : DataProvider {
    val workListValues: StateFlow<WorkResponse?>
    val articlesValue: StateFlow<List<ArticlesItem>>
    val states: StateFlow<Result<WorkResponse>>
}

class WorkListDataProviderImpl : WorkListDataProvider {

    override val workListValues = MutableStateFlow<WorkResponse?>(null)

    override val articlesValue = MutableStateFlow<List<ArticlesItem>>(emptyList())
